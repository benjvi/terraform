...
func createEgressFirewallRule(
	...
	if rule["protocol"].(string) == "icmp" {
		...
		r, err := cs.Firewall.CreateEgressFirewallRule(p, true)
		if err != nil {
			return err
		}
		uuids["icmp"] = r.Id
...
func resourceCloudStackEgressFirewallRead(d *schema.ResourceData, meta interface{}) error {
	...
	ruleMap := make(map[string]*cloudstack43.EgressFirewallRule, l.Count)
...
func deleteEgressFirewallRule(
	...
	for k, id := range uuids {
		...
		// Delete the rule
		if _, err := cs.Firewall.DeleteEgressFirewallRule(p, true); err != nil {
