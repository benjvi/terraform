...
func setTags(cs *cloudstack43.CloudStackClient, d *schema.ResourceData, resourcetype string) error {
	...
	if len(remove) > 0 {
		...
		p.SetTags(remove)
		_, err := cs.Resourcetags.DeleteTags(p, true)
		if err != nil {
			return err
		}
	}
	...
	if len(create) > 0 {
		...
		_, err := cs.Resourcetags.CreateTags(p, true)
		if err != nil {
			return err
		}
	}
